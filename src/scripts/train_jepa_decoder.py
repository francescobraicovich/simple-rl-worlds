...
class JEPADecoderTrainer:
    ...
    def __init__(self, config_path: str = None):
        ...
        # Training parameters
        self.training_config = self.config['training']['jepa_decoder']
    ...
    def initialize_optimizer(self):
        """
        Initialize the AdamW optimizer and learning rate scheduler only for the decoder's trainable parameters.
        """
    ...
    def train_step(self, batch: Tuple[torch.Tensor, ...]) -> float:
        ...
        with torch.no_grad():
            ...
            # 2. Predict next latent state using current latent state and full action sequence
            z_next_pred = self.predictor(z_state.detach(), action) # Detach z_state
