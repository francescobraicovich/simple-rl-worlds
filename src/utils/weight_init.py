...
def initialize_weights(module):
    """
    Initializes weights for layers in a PyTorch module with improved numerical stability.
    ...
    for m in module.modules():
        if isinstance(m, nn.Conv2d):
            ...
            # More conservative initialization for linear layers to prevent explosions
            if hasattr(m, 'weight') and m.weight is not None:
                # Use Xavier/Glorot for attention layers (better for transformers)
                if m.weight.shape[0] >= 512 or m.weight.shape[1] >= 512:  # Large attention layers
            ...
            # Initialize embedding layers with a normal distribution (smaller std dev)
            nn.init.normal_(m.weight, mean=0, std=0.01)  # Reduced from 0.02
