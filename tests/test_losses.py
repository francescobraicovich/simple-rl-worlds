...
class TestLosses(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Dummy inputs
        self.z1 = torch.randn(self.batch_size, self.embed_dim, device=self.device)
        self.z2 = torch.randn(self.batch_size, self.embed_dim, device=self.device)
        self.z_requires_grad = torch.randn(self.batch_size, self.embed_dim, device=self.device, requires_grad=True)

    # --- VICRegLoss Tests ---
    def test_vicreg_instantiation(self):
        loss_fn = VICRegLoss().to(self.device)
        self.assertIsInstance(loss_fn, nn.Module)
        loss_fn_custom = VICRegLoss(sim_coeff=10.0, std_coeff=20.0, cov_coeff=0.5, eps=1e-5).to(self.device)
    ...
    def test_vicreg_forward_and_reg_terms(self):
        ...
        self.assertTrue(loss.requires_grad == False) # Inputs dont require grad by default in setUp

        loss_grad, _, _, _ = loss_fn(self.z_requires_grad, self.z_requires_grad + 0.1)
    ...
    def test_vicreg_std_loss_behavior(self):
        loss_fn = VICRegLoss(std_coeff=1.0, cov_coeff=0.0).to(self.device) # Focus on std_loss
        # Input with std close to 1 for all features
        z_std_one = torch.randn(self.batch_size, self.embed_dim, device=self.device)
        z_std_one = (z_std_one - z_std_one.mean(dim=0)) / (z_std_one.std(dim=0) + 1e-5) # Normalize features
        ...
        z_std_zero = torch.zeros(self.batch_size, self.embed_dim, device=self.device)
        reg_loss_zero_std, std_loss_val_zero, _ = loss_fn.calculate_reg_terms(z_std_zero)
        # Hinge loss max(0, 1-0) = 1. Mean over features should be 1.
        self.assertAlmostEqual(std_loss_val_zero.item(), 1.0 * loss_fn.std_coeff, delta=1e-3)


    def test_vicreg_cov_loss_behavior(self):
        loss_fn = VICRegLoss(std_coeff=0.0, cov_coeff=1.0).to(self.device) # Focus on cov_loss
        ...
        z_uncorr = torch.randn(self.batch_size, self.embed_dim, device=self.device)
    ...
    def test_barlowtwins_instantiation(self):
        ...
        self.assertIsInstance(loss_fn, nn.Module)
        loss_fn_custom = BarlowTwinsLoss(lambda_param=0.01, eps=1e-6, scale_loss=0.5).to(self.device)
        self.assertIsInstance(loss_fn_custom, nn.Module)
    ...
    def test_barlowtwins_ideal_input(self):
        ...
        z_ideal_norm = torch.eye(self.batch_size, self.embed_dim, device=self.device)
        if self.batch_size > self.embed_dim: # More samples than features
             # Make columns orthonormal (approx)
            q, _ = torch.linalg.qr(torch.randn(self.batch_size, self.embed_dim, device=self.device))
            z_ideal_norm = q * (self.batch_size**0.5) # scaled so z_norm.T @ z_norm / N = I
        elif self.batch_size < self.embed_dim: # More features than samples
            # Make rows orthonormal (approx)
            q, _ = torch.linalg.qr(torch.randn(self.embed_dim, self.batch_size, device=self.device))
            z_ideal_norm = q.T * (self.batch_size**0.5)
        else: # batch_size == embed_dim
            q, _ = torch.linalg.qr(torch.randn(self.batch_size, self.batch_size, device=self.device))
        ...
        off_diag_ideal = loss_fn._off_diagonal(identity_corr) # from barlow_twins.py
        ...
        z_pre_normalized = torch.randn(self.batch_size, self.embed_dim, device=self.device)
        z_pre_normalized = (z_pre_normalized - z_pre_normalized.mean(dim=0)) / (z_pre_normalized.std(dim=0) + 1e-5)
        ...
        self.assertAlmostEqual(inv_l.item() / loss_fn.scale_loss, 0.0, delta=1e-3, msg="Invariance loss for pre-normalized input not near zero.")
    ...
    def test_dino_instantiation(self):
        ...
        self.assertIsInstance(loss_fn, nn.Module)
        self.assertTrue(torch.allclose(loss_fn.center, torch.zeros(1, self.embed_dim, device=self.device)))
        loss_fn_custom = DINOLoss(out_dim=self.embed_dim, center_ema_decay=0.95).to(self.device)
        self.assertIsInstance(loss_fn_custom, nn.Module)
    ...
    def test_dino_center_update(self):
        loss_fn = DINOLoss(out_dim=self.embed_dim, center_ema_decay=0.9).to(self.device)
        initial_center = loss_fn.center.clone()

        # Train mode: center should update
        loss_fn.train()
        z_train = torch.ones(self.batch_size, self.embed_dim, device=self.device) * 2.0 # Mean is 2.0
        ...
        self.assertTrue(torch.allclose(updated_center_train, torch.ones_like(initial_center) * 0.2, atol=1e-5))

        # Eval mode: center should NOT update
        loss_fn.eval()
        center_before_eval = loss_fn.center.clone()
        z_eval = torch.ones(self.batch_size, self.embed_dim, device=self.device) * 5.0 # Mean is 5.0
        ...
        z_batch1 = torch.ones(self.batch_size, self.embed_dim, device=self.device) * 1.0 # Mean 1
        ...
        z_batch2 = torch.ones(self.batch_size, self.embed_dim, device=self.device) * 1.0 # Mean 1
        # Expected loss: (1.0 - 0.1)^2 * embed_dim = (0.9)^2 * embed_dim
        loss2, _, _ = loss_fn.calculate_reg_terms(z_batch2)
        self.assertAlmostEqual(loss2.item(), (0.9**2) * self.embed_dim, delta=1e-5)


    def test_dino_input_validation(self):
        loss_fn = DINOLoss(out_dim=self.embed_dim).to(self.device)
        with self.assertRaises(ValueError):
            loss_fn(torch.randn(self.batch_size, self.embed_dim + 10, device=self.device))
        with self.assertRaises(ValueError):
            loss_fn(torch.randn(self.batch_size, self.embed_dim, 1, device=self.device))
